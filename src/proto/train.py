...
if not os.path.exists(PATH+"/models/proto"):
    os.makedirs(PATH+"/models/proto")
    os.makedirs(PATH+"/data/")
    os.makedirs(PATH+"/output/")

def train(dataloader, model, optimizer, suffix, num_batches=100):
    global PATH
    avg_loss = list()
    avg_accuracy = list()
    for i, batch in enumerate(dataloader):
        ...
        log_prob = model(train_inputs, train_targets, test_inputs)
        loss = F.cross_entropy(log_prob, test_targets)
        ...
        if i % 10 == 0:
            test_predictions = torch.argmax(log_prob, dim=1)
            ...
            torch.save(net.state_dict(), PATH+"/models/proto/proto-train-{}.pth".format(suffix))
...
if __name__ == "__main__":
    ...
    #TODO: Why doesn't this work with 1-shot? That seems strange?
    ...
    if args.train:
        ...
        for t in range(epochs):
            print(f"Epoch {t+1}\n-------------------------------")
            loss, accuracy = train(dataloader, net, optimizer, suffix, num_batches=200)

            #Save loss to a file
            track_loss.append((loss, accuracy))
            df = pd.DataFrame(track_loss, columns=['Loss', 'Accuracy'])
            df.to_csv(PATH+"/output/proto_final_loss_{}.csv".format(suffix))

        print("Done!")
        print('Finished Training')

        #Save loss over time and network weights
        filePath = PATH+"/models/proto/proto-final-{}.pth".format(suffix)
        torch.save(net.state_dict(), filePath)
        print("Saved PyTorch Model State to {}".format(filePath))
    else:
        #Test neural network
        if os.path.exists(PATH+"/models/proto/proto-final-{}.pth".format(suffix)):
            net = ProtoNetwork(1, 64)
            net.load_state_dict(torch.load(PATH+"/models/proto/proto-final-{}.pth".format(suffix)))
            if torch.cuda.is_available(): 
                net.cuda()
