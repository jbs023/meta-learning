...
def matching_log_probas(embeddings, targets, test_embeddings, num_classes, eps=1e-8):
    ...
    value = torch.log(sum_exp) + max_similarities - logsumexp
    return value
   

class MatchingNetwork(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_size=64, way=5):
    ...
    def forward(self, train_inputs, train_targets, test_inputs):
        #Collapse multiple batches into one
        train_data = train_inputs.view(-1, *train_inputs.shape[2:]) 
        train_embedding = self.encoder(train_data)

        #Expand batches
        train_embedding = train_embedding.view(*train_inputs.shape[:2], -1)

        #Collapse multiple batches into one
        test_data = test_inputs.view(-1, *test_inputs.shape[2:])
        test_embedding = self.encoder(test_data)

        #Expand batches
        test_embedding = test_embedding.view(*test_inputs.shape[:2], -1)

        log_probas = matching_log_probas(train_embedding, train_targets, test_embedding, self.way)
